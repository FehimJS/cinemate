...
const popularMovies = useState("popular-movies", () => null);
const popularSeries = useState("popular-series", () => null);
if (!popularMovies.value && !popularSeries.value) {
	const { data, error } = await useFetch("/api/popular", {
		getCachedData: (key) => {
		...
		<section class="mt-12">
			<h1
				...
				<span
					...
					>Popüler Diziler</span
				>
			</h1>
			<div class="flex justify-center mx-8">
				<ul
					class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 grid-flow-row-dense auto-rows-auto gap-4">
					<li v-for="serie in popularSeries">
						<SeriesCard
							:id="serie.id"
							:name="serie.name"
